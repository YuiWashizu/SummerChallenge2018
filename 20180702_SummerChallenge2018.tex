...
\part{目的}
あ光には、波動性と粒子性の2重性がある。ある時は粒子のようにふるまい、ある時は波のようにふるまう。今回の実験では、日常概念での「粒子」や「波動」をそのまま当てはめることのできない光子の性質を確かめる。
\clearpage
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=7cm, bb = 0 0 600 350]{SummerChallenge_lineray.png}
\caption{線状光源}
\end{center}
\end{figure}
...
\]
となる。ここで、$r = r(y)=\sqrt{R^2 cos^2 \theta + (Rsin \theta - y )^2}$である。
...
$r(y)$を$y$でテイラー展開すると、
\begin{eqnarray*}
r &=&r(0) + \frac{\partial r(0)}{\partial y} y + \frac{1}{2!} \frac{\partial^2 r(0)}{\partial y^2} y^2 +  …\\
&=&R - ysin\theta + \frac{y^2}{2R} cos^2 \theta
\end{eqnarray*}
...
\begin{eqnarray*}
E &=& C_L \int_{-D/2}^{D/2} \frac{sin(\omega t - k( R - ysin\theta ))}{R} dy \\
